...
Продемонструвати повторюваний патерн роботи CLI-агента:
...
| Порожній рядок обробляється | ✅ |
| Рядок із самих пробілів не ламається | ✅ |
| Усі наявні тести зелені | ✅ 20/20 |
| Нема регресій | ✅ 0 |
...
| **Результат** | 1 коміт + готовий до PR |
...
Провал на першому regex (`\b\w`) показав, що навіть простий функціонал потребує критичного тестування на край-кейсах (hyphenated words). Другий цикл — швидке переписування регулярного виразу для коректної поведінки.
